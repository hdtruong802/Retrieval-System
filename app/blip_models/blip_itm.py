from app.blip_models.med import BertConfig, BertModel
...
from app.blip_models.blip import create_vit, init_tokenizer, load_checkpoint
...
class BLIP_ITM(nn.Module):
    def __init__(self,                
                 ...
                 vit_ckpt_layer = 0,                      
                 embed_dim = 256,     
                 keyframes_dict = "E:/AIC/blip_bin_json/faiss_cosine_1.bin",
                 features_path = "E:/AIC/blip_bin_json/keyframes_id_all_1.json"
                 ):
        """
        Args:
    ...
    def text_search(self, text, k):
        ...
        image_paths = [self._resolve_image_path(p) for p in image_paths]
    ...
    def _resolve_image_path(self, input_path: str) -> str:
        # Normalize slashes and trim leading separators
        normalized = input_path.replace('\\', '/').lstrip('/')
        # If already absolute (Windows drive or POSIX), return as forward-slashed
        if re.match(r'^[A-Za-z]:/', normalized) or input_path.startswith('/'):
            return normalized
        # Expect JSON to start with 'Data/...'; map to actual 'data/images/...'
        if normalized.lower().startswith('data/'):
            parts = normalized.split('/')
            if len(parts) >= 2 and parts[1].lower() != 'images':
                parts.insert(1, 'images')
            normalized = '/'.join(parts)
        # Make absolute under project root
        absolute_path = os.path.join(WORK_DIR, normalized)
